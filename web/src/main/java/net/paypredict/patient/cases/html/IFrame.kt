package net.paypredict.patient.cases.html

import com.vaadin.flow.component.HtmlContainer
import com.vaadin.flow.component.PropertyDescriptor
import com.vaadin.flow.component.PropertyDescriptors
import com.vaadin.flow.component.Tag
import com.vaadin.flow.server.AbstractStreamResource
import com.vaadin.flow.server.StreamResource

import java.util.Optional

/**
 *
 *
 * Created by alexei.vylegzhanin@gmail.com on 9/21/2018.
 */
@Tag("iframe")
class IFrame(src: String = "about:blank") : HtmlContainer() {
    var src: String
        get() = get(srcDescriptor)
        set(src) = set(srcDescriptor, src)

    init {
        this.src = src
        set(frameBorderDescriptor, "0")
    }

    fun setSrc(src: AbstractStreamResource) {
        element.setAttribute("src", src)
    }

    companion object {
        private val srcDescriptor =
            PropertyDescriptors.attributeWithDefault("src", "")
        private val frameBorderDescriptor =
            PropertyDescriptors.attributeWithDefault("frameborder", "")
    }
}
